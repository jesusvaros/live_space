...
export const useHomeSuggestions = (events: EventListItem[], options: { startOfToday: Date; endOfEventsWindow: Date }) => {
  const eventsKey = useMemo(
    () => events.map(event => `${event.id}:${event.starts_at}`).sort().join('|'),
    [events]
  );

  const { data, loading } = useQuery<{ suggestedArtists: SuggestedArtist[]; suggestedVenues: SuggestedVenue[] }>(
    ['events:home_suggestions', eventsKey, options.startOfToday.toISOString(), options.endOfEventsWindow.toISOString()],
    async () => {
      ...
      const suggestedArtists = await (async () => {
      ...
      const suggestedVenues = await cached(
      ...
      return { suggestedArtists, suggestedVenues };
    },
    {
      enabled: events.length > 0,
      ttlMs: 15_000,
      initialData: { suggestedArtists: [], suggestedVenues: [] },
    }
  );

  return {
    suggestedArtists: data?.suggestedArtists || [],
    suggestedVenues: data?.suggestedVenues || [],
    loading: events.length > 0 ? loading : false,
  };
};
