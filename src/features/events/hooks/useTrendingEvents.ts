...
export const useTrendingEvents = (events: EventListItem[], startOfToday: Date) => {
  ...
  const { data, loading } = useQuery<{ trending: EventListItem[]; meta: TrendingMeta }>(
    ['events:trending', eventsKey, startOfToday.toISOString()],
    async () => {
      ...
      for (const eventId of rankedIds) {
        ...
        if (!item) continue;
        meta[eventId] = { saves: item.saves, going: item.going, attended: item.attended, moments: item.moments };
      }
